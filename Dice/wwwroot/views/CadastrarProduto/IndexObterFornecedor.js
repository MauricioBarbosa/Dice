const Fabricantes = {
    ...
    carregarFabricantes() {
        ...
        fetch("/CadastrarProduto/carregarFabricantes?pesquisa=" + pesquisa, config)
            .then((dadosJson) => {
...
onclick="Fabricantes.setFabricante({id : ${dadosObj[i].id},nome: '${dadosObj[i].nome}'})"><i class="fas fa-check-circle"></i></button></th>
                </tr>`
                    }
    ...
    setFabricante: function (fabricante) {
        window.parent.fabricante = fabricante;
        window.parent.document.getElementById("fabricanteProduto").value = fabricante.nome;
        console.log(window.parent.fabricante)
        window.parent.$.fancybox.close();
    }

}

Fabricantes.carregarFabricantes();